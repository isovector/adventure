return function(room)
    local hotspot = Hotspot.new(nil, nil, nil,
        550, 406, 571, 404, 584, 420, 584, 439, 574, 454, 566, 454, 570, 476, 573, 514, 571, 540, 567, 572, 567, 598, 553, 599, 553, 562, 556, 535, 554, 517, 550, 488, 551, 454, 547, 452, 536, 438, 534, 415)

    local src = MOAIImageTexture.new()
    src:load(room.img_path, MOAIImage.TRUECOLOR + MOAIImage.PREMULTIPLY_ALPHA)
    
    local sx, sy, dx, dy = hotspot.shape:getBBox()
    ...
    for y = 1, h do
        for x = 1, w do
            if hotspot:hitTest(x + sx, y + sy) then
