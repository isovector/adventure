...
mrequire "src/class"
require "src/engine/fragmentshader"
...
-- Both vsh and fsh should be basenames of files in /game/shaders.
...
function Shader.registerVsh(id)
    file = io.open(string.format("game/shaders/%s.vsh", id))
...
function Shader.registerFsh(id)
    file = io.open(string.format("game/shaders/%s.fsh", id))
    local shader = file:read('*all')
    file:close()
